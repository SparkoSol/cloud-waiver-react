...
const FormRender = () => {
  ...
  useEffect(() => {
    if (waiver) {
      $(fb.current).formRender({
        formData: JSON.stringify(waiver?.form_data),
        ...options,
      });
  ...
  useEffect(() => {
    setLoading(true);
    dispatch(getPublicWaiver(id)).finally(() => setLoading(false));
    // eslint-disable-next-line
  }, []);
    ...
    for (let item of htmlArr) {
      ...
      switch (item.type) {
        case "signature":
          ...
          const isDirty = $(signNode.closest(".formbuilder-signature"))
            .find(".js-signature")
            .attr("data-dirty");
          label = signNode.closest(".formbuilder-signature").firstChild
            .lastChild?.classList;
          if (!isDirty && label) {
        ...
        case "address":
          let signatureComponent = document.querySelectorAll(
            ".adult .js-signature"
          );
          let breakLoop = false;
          signatureComponent = $(
            signatureComponent[tracker.primaryAdultParticipantCount]
          );
          let formElements = document.querySelectorAll(
            `${
              item.type === "primaryAdultParticipant" ? "#myForm" : "#address"
            }`
          );
          formElements =
            formElements[
              item.type === "primaryAdultParticipant"
        ...
        case "additionalMinors":
          let finalArr = [];
          let allForms;
          if (item.type === "additionalParticipants")
            allForms =
              document.querySelectorAll(".participant-div-1")[
                tracker.additionalParticipantsCount
              ];
          else
            allForms =
              document.querySelectorAll(".minor-div-1")[
            ...
            for (let form of allForms.childNodes) {
              let signature = $(
                document.querySelectorAll(
                  `.${form.className.replace(/ /g, ".")} .js-signature`
                )[tracker.additionalParticipantsCount]
              );
              let temp = {};
          ...
          tracker[
            `${
              item.type === "additionalParticipants"
        ...
        case "capturePhoto":
          ...
          const parentNode = document.querySelectorAll(
            ".formbuilder-capturePhoto-label"
          )[tracker.capturePhotoCount];
        ...
        case "electronicSignatureConsent":
          let checkboxDiv =
            document.querySelectorAll("#electronicSign")[
              tracker.electronicSignatureConsentCount
            ];
          let checkbox = checkboxDiv.checked;
          label = checkboxDiv.closest(`.formbuilder-electronicSignatureConsent`)
            .firstChild.lastChild?.classList;
          if (!checkbox && label) {
        ...
        case "richTextEditor":
          ...
          let itemData = document.querySelector(
            'div[role="application"] table'
          );
        ...
        case "filesUpload":
          const fileInp =
            document.querySelectorAll(".file-inp")[tracker.fileUpload];
          label = fileInp.closest(".formbuilder-filesUpload").firstChild
            .lastChild?.classList;
          const urlArr = [];
        ...
        case "timeComponent":
          const allTimeDivs =
            document.querySelectorAll("#time")[tracker.timeCount];
        ...
        case "dateOfBirth":
          ...
          const dob = document.querySelectorAll(`input[name='dateOfBirth']`)[
  ...
  return (
    <div className="max-w-5xl mx-auto my-6 common p-4">
      <p className="text-sm my-6">
        Reference No :{" "}
        <span
          ref={refNo}
        >{`${getDynamicTenantId().toUpperCase()}.${today()}.${Math.floor(
          Math.random() * 1000000
        )}`}</span>
      </p>
      <form ref={fb}></form>
