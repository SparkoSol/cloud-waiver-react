  ...
  return `<form class="space-y-2" id="myForm">
        ...
        <div class="absolute inset-0 flex items-center justify-center cursor-pointer" onclick="this.remove();">
          ...
          &nbsp;&nbsp;Click to sign</h5>
            ...
            <button type="button" onclick="$(this.parentNode).find('.js-signature').jqSignature('clearCanvas');" class="w-7 h-7 justify-center items-center border border-gray-500 ml-auto text-gray-500 rounded-full hidden" id="reset">X</button>
    ...
    return {
      onRender: function () {
       ...
       <button type="button" id="reset-sign"
        onclick="$(this.parentNode).find('.js-signature').jqSignature('clearCanvas');"
         class="w-7 h-7 justify-center items-center border border-gray-500 ml-auto text-gray-500 rounded-full hidden">X</button>
        <div class="js-signature"></div>
        </div>
      `);
        $('.js-signature').jqSignature({autoFit: true, height: 200, border: '1px dashed #D1D5DB', lineWidth: 3});
        $('.js-signature').on('jq.signature.changed', function() {
          $('#reset-sign').css('display', 'flex');
        });
      }
    };
  },
